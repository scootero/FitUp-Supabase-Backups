...
serve(async (request)=>{
  ...
  try {
    const { data: activeRows, error: activeError } = await supabaseAdmin.from("matches").select("id, metric_type, duration_days, scoring_mode").eq("state", "active");
    if (activeError) {
      throw activeError;
    }
    ...
    for (const match of matches){
      const { data: participantRows, error: participantError } = await supabaseAdmin.from("match_participants").select("user_id, baseline_steps").eq("match_id", match.id);
      if (participantError) {
