...
CREATE POLICY "analytics_events: insert" ON "public"."analytics_events" FOR INSERT WITH CHECK (((("user_id" IS NULL) AND ("auth"."uid"() IS NULL) AND ("event_name" = ANY (ARRAY['app_cold_start'::"text", 'auth_screen_view'::"text", 'app_opened'::"text", 'app_backgrounded'::"text", 'session_started'::"text", 'session_ended'::"text"]))) OR (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
