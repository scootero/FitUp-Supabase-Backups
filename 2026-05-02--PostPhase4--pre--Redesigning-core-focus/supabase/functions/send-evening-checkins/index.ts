...
async function pickClosestActiveMatchForUser(userId) {
  const { data: rows, error } = await supabaseAdmin.from("match_participants").select("match_id, matches!inner(id, state, metric_type, duration_days)").eq("user_id", userId);
  if (error) {
    throw error;
  }
  ...
  for (const row of active){
    ...
    const { data: pRows, error: pErr } = await supabaseAdmin.from("match_participants").select("user_id").eq("match_id", matchId);
    if (pErr) {
      throw pErr;
    }
    const parts = pRows ?? [];
